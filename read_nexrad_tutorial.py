...
def plot_radar_images(plots):
    ...
    for plotno, plot in enumerate(plots, start=1):
        ...
        display.plot(plot[0], plot[2], ax=ax, title=plot[1], colorbar_label='',
...
# variable-name in pyart, display-name that we want, sweep-number of radar
# (0=lowest ref, 1=lowest velocity)
plots = [['reflectivity', 'Reflectivity (dBZ)', 0],
